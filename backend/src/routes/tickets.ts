...
// Ticket photos now arrive as Cloudinary public ids (ADR 002): the browser
// uploads directly to Cloudinary with a signature from `/api/v1/uploads/signature`
// and this route only records the result. TICKET_UPLOAD_DIR survives solely to
// keep serving photos uploaded before that change - see GET /photo/:filename.
...
// and the assigned vendor (so the frontend can offer the manual WhatsApp link).
...
router.patch('/:id', requireAuth, loadMembership, requireRole('update', 'ticket'), async (req, res, next) => {
  try {
    ...
    // ── Vendor resolution ────────────────────────────────────────────────────
    // The assignment form always sends a real Vendor id (picked from the
    // autocomplete or created inline). It is resolved tenant-scoped here, so a
    // vendor id from another society can never be attached to this ticket. The
    // vendor's name is snapshotted onto `assignedTo` for the vendor rating
    // aggregation, which groups by name. Free-text `assignedTo` (no vendorId)
    // stays supported for legacy callers and detaches any vendor record.
    let vendorRecord: { id: string; name: string; phone: string | null; email: string | null } | null = null;
    if (input.vendorId) {
      vendorRecord = await prisma.vendor.findFirst({
        where: { id: input.vendorId, societyId, deletedAt: null },
        select: { id: true, name: true, phone: true, email: true },
      });
      if (!vendorRecord) {
        throw new AppError(ErrorCodes.VALIDATION_ERROR, 400, 'Vendor not found for this society');
      }
    }

    let vendorTargetId = existing.vendorId ?? null;
    if (vendorRecord) {
      vendorTargetId = vendorRecord.id;
      updateData.assignedTo = vendorRecord.name;
    } else if (input.assignedTo !== undefined || input.vendorId === null) {
      vendorTargetId = null;
      if (input.assignedTo !== undefined) updateData.assignedTo = input.assignedTo;
    }
    if (vendorTargetId !== (existing.vendorId ?? null)) {
      updateData.vendorId = vendorTargetId;
    }
    ...
    const effectiveVendor = vendorRecord
      ? vendorRecord.name
      : (input.assignedTo !== undefined ? input.assignedTo : existing.assignedTo);
    const assignmentChanged =
      effectiveVendor !== existing.assignedTo || vendorTargetId !== (existing.vendorId ?? null);
    // Never fall back to the *previous* vendor's address when the vendor
    // changes - that would email one vendor another vendor's job. A newly
    // selected Vendor record may already have a contact email on file, which is
    // the right address to send the job link to.
    const effectiveVendorEmail = input.vendorEmail !== undefined
      ? input.vendorEmail
      : (assignmentChanged ? (vendorRecord?.email ?? null) : existing.vendorEmail);
    const vendorEmailChanged = effectiveVendorEmail !== existing.vendorEmail;

    // Can this vendor be reached at all? Email is the automatic channel; the
    // phone is the manual one. A vendor with *either* gets a job link - the link
    // is channel-independent, and that is what lets the admin's WhatsApp button
    // carry it for a phone-only vendor that was never emailed (ADR 007).
    const vendorPhoneOnFile = vendorRecord?.phone ?? null;
    const vendorReachable = Boolean(effectiveVendor && (effectiveVendorEmail || vendorPhoneOnFile));
    ...
    if ((assignmentChanged || vendorEmailChanged) && !closingNow && existing.status !== 'CLOSED') {
      if (vendorReachable) {
        ...
        // Only advertise an address we actually email: a phone-only vendor keeps
        // `vendorEmail` null, so the response can tell "not emailed" apart from
        // "emailed", and the UI can prompt for a manual WhatsApp send.
        updateData.vendorEmail = effectiveVendorEmail ?? null;
      } else {
        // No usable contact of any kind - issue no link, and revoke any stale one
        // so it can't keep granting access to an unassigned vendor. The stored
        // email is cleared too, so the row can't advertise a contact the job
        // link was never sent to.
        updateData.vendorEmail = null;
    ...
    // (ADR 004), never Nodemailer directly. The same URL is handed back to the
    // admin in this response so the manual "Message on WhatsApp" button can
    // reuse it verbatim - one link, two delivery channels, no second token.
    //
    // A vendor with no email is never emailed. That is not an error: the
    // assignment stands, no link is quietly sent, and the response still carries
    // the URL so the admin can hand it over on WhatsApp.
    let vendorTicketUrl: string | undefined;
    if (vendorAccessToken) {
      vendorTicketUrl = buildVendorTicketUrl(vendorAccessToken);
    }
    let vendorLinkSent = false;
    if (vendorAccessToken && vendorTicketUrl && ticket.assignedTo && ticket.vendorEmail) {
    ...
    await logAudit({
      ...
      before: { status: existing.status, assignedTo: existing.assignedTo, vendorId: existing.vendorId },
      after: {
    ...
    // `vendorTicketUrl` is present whenever this response issued a link, and
    // `vendorLinkSent` is true only when that link was actually emailed. The two
    // come apart for a phone-only vendor: a link to hand over, nothing sent.
    const response: TicketResponse = vendorTicketUrl
      ? {
          ...formatTicket(ticket),
          vendorLinkSent,
          vendorTicketUrl,
...
// ── POST /api/v1/tickets/:id/photos ────────────────────────────────────────
// Records photos that already sit in Cloudinary (uploaded by the browser using a
// signature issued for this ticket). Each public id is re-resolved through the
// provider, which rejects anything outside this ticket's own tenant folder.
router.post('/:id/photos', requireAuth, loadMembership, async (req, res, next) => {
  try {
    const societyId = req.membership!.societyId;
    const isAdmin = req.membership!.role === 'COMMITTEE_ADMIN' || req.membership!.role === 'SUPER_ADMIN';

    const input = AttachTicketPhotosSchema.parse(req.body);
    ...
    const expectedFolder = storageFolder(societyId, UPLOAD_PURPOSE_CONFIG['ticket-photo'].resourceType, ticket.id);

    const confirmed = [];
    for (const publicId of input.publicIds) {
      confirmed.push(
        await cover.confirmUpload({
          publicId,
          expectedFolder,
          allowedFormats: UPLOAD_PURPOSE_CONFIG['ticket-photo'].allowedFormats,
          maxFileSizeBytes: UPLOAD_PURPOSE_CONFIG['ticket-photo'].maxFileSizeBytes,
        })
      );
    }
    const photoUrls = confirmed.map((c) => c.url);

    // Append to existing photosUrl. Legacy entries are relative API paths served
    // by GET /photo/:filename; new ones are Cloudinary delivery URLs.
    ...
    await logAudit({
      ...
      entityType: 'ticket', entityId: ticket.id,
      after: { photoCount: allUrls.length, publicIds: confirmed.map((c) => c.publicId) },
    });

    sendSuccess(res, { urls: photoUrls, publicIds: confirmed.map((c) => c.publicId) }, 201);
  } catch (err) { next(err); }
});
